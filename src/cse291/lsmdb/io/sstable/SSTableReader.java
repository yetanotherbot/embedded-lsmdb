...
public class SSTableReader {
    ...
    private File dir;
    private MemDataLoader level0DataLoader;
    ...
    public String getByLevel(String row, String col, int level) throws IOException, NoSuchElementException {
        ...
        FilterLoader filterLoader = new FilterLoader(filterFile, hasher);
        filterLoader.loadInMemory();
        if (!filterLoader.isPresent(row)) {
        ...
        IndexLoader indexLoader = new IndexLoader(indexFile);
        indexLoader.loadInMemory();
        int blockIndex = indexLoader.lookup(new RowCol(row, col));
        if (blockIndex < 0) {
            throw new NoSuchElementException(String.format(
                    "index reject: row column pair is not in the cuerrent level: row %, col %s, level %d",
                    row, col, level
            ));
        }
